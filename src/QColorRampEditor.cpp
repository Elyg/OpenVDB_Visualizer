    ...
    visText_(false), textColor_(Qt::white), textAcc_(1)
{
    ...
    QColor myBlue = QColor(0.2*255, 0, 1*255);
    QColor myCyan = QColor(0, 0.85*255, 1*255);
    QColor myGreen = QColor(0, 1*255, 0.1*255);
    QColor myYellow = QColor(0.95*255, 1*255, 0);
    QColor myRed = QColor(255, 0, 0);
    
    
    QVector<QPair<qreal, QColor> > ramp;
    ramp.push_back(QPair<qreal, QColor>(0.0, myBlue));
    ramp.push_back(QPair<qreal, QColor>(0.25, myCyan));
    ramp.push_back(QPair<qreal, QColor>(0.5, myGreen));
    ramp.push_back(QPair<qreal, QColor>(0.75, myYellow));
    ramp.push_back(QPair<qreal, QColor>(1.0, myRed));
...
// -----------------------------------------------------------
std::vector<float> QColorRampEditor::getColorTable()
{
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        _ctable.push_back(red);
        _ctable.push_back(green);
        _ctable.push_back(blue);
    }
    return _ctable;
}
// -----------------------------------------------------------
